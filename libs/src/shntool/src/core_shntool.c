...
int run_split(char* s) {
    char** options = malloc(sizeof(char*));
    char** substrings = malloc(sizeof(char*));
    int count = 0;
    char delim[] = "\"";
	char *ptr = strtok(s, delim);
    substrings[count] = malloc(sizeof(char) * strlen(ptr));
    strcpy(substrings[count], ptr);
    count++;
	while(ptr != NULL)
	{
    ...
    for (int i=0;i<count;i++) {
        ...
        if (i % 2 == 0) {
            int lc = 0;
            char* copy = malloc(sizeof(char) * strlen(substrings[i]));
            strcpy(copy, substrings[i]);

            char d[] = " ";
            ptr = strtok(copy, d);
            options = realloc(options, (all + 1) * sizeof(char*));
            options[all] = malloc(strlen(ptr)* sizeof(char));
            strcpy(options[all], ptr);
            all++;
            ...
            options = realloc(options, (all + 1) * sizeof(char*));
            options[all] = malloc(strlen(substrings[i])* sizeof(char));
            strcpy(options[all], substrings[i]);
            all++;
        }
    }
    mysplit(all, options);
...
//    char s[] = "shntool split -f \"Bucks Fizz - Hand Cut - 1983 - LP.cue\" -o  flac  \"Bucks Fizz - Hand Cut - 1983 - LP1.flac\"";
//    int count = run_split(s);
//    return 0;
//}
